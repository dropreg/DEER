...
class Linear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True, before = False,
        ...
        self.before = before
        if self.before:
            self.mask_scores = torch.nn.Parameter(torch.Tensor(self.out_features), requires_grad=True)
        else:
            self.mask_scores = torch.nn.Parameter(torch.Tensor(self.in_features), requires_grad=True)
    ...
    def get_num_parameter(self):
        _score = self.mask_scores.detach()
        if self.before:
            _score = torch.repeat_interleave(_score.unsqueeze(-1), self.in_features, dim=-1)
        else:
            _score = torch.repeat_interleave(_score.unsqueeze(0), self.out_features, dim=0)
        return _score.type(self.weight.type())

    def get_mask(self):
        _mask = ThresholdBinarizer.apply(self.mask_scores, self.threshold)
        
        if self.before:
            _mask = torch.repeat_interleave(_mask.unsqueeze(-1), self.in_features, dim=-1)
        else:
            _mask = torch.repeat_interleave(_mask.unsqueeze(0), self.out_features, dim=0)
        return _mask.type(self.weight.type())
    ...
    def get_weight_bias(self):
        ...
        #     print("Linear", _mask.view(-1).sum().data, _mask.view(-1).size(0), \
        #          _mask.view(-1).sum()/_mask.view(-1).size(0))
        
        _weight = self.weight * _mask
    ...
    def forward(self, input: Tensor) -> Tensor:
        if len(self.cache) > 0 and not self.training:
            _weight = self.cache["weight"]
            _bias = self.cache["bias"]
        else:
            _weight, _bias = self.get_weight_bias()
            self.cache["weight"] = _weight
            self.cache["bias"] = _bias
    ...
    #              _mask.view(-1).sum()/_mask.view(-1).size(0))
    #         if self.before:
    #             _weight = self.weight.masked_select(_mask.bool()).view(-1, self.weight.size(1))
    #             _bias = self.bias.masked_select(ThresholdBinarizer.apply(self.mask_scores, self.threshold).bool())
    #         else:
    #             _weight = self.weight.masked_select(_mask.bool()).view(self.weight.size(0), -1)
    #             _bias = self.bias
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        prefix = name + ".mask_scores"
        if prefix not in state_dict:
            state_dict[prefix] = self.mask_scores
...
class BlockLinear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True,
        ...
        self.mask_scores = torch.nn.Parameter(torch.Tensor(col_size, row_size), requires_grad=True)
        torch.nn.init.constant_(self.mask_scores, val=0.0)
    ...
    def get_num_parameter(self):
        _score = self.mask_scores
        _score = torch.repeat_interleave(_score, self.block_size, dim=0)
        _score = torch.repeat_interleave(_score, self.block_size, dim=1)
        return _score.type(self.weight.type())

    def get_mask(self):
        _mask = ThresholdBinarizer.apply(self.mask_scores, self.threshold)
        _mask = torch.repeat_interleave(_mask, self.block_size, dim=0)
        _mask = torch.repeat_interleave(_mask, self.block_size, dim=1)
        return _mask.type(self.weight.type())
    ...
    def get_weight_bias(self):
        ...
        # if not self.training:
        #     print("Block Linear", _mask.view(-1).sum(), _mask.view(-1).size(0), _mask.view(-1).sum()/_mask.view(-1).size(0))
...
class MultiheadAttention(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        self.k_proj = BlockLinear(self.kdim, embed_dim, bias=bias)
        self.v_proj = BlockLinear(self.vdim, embed_dim, bias=bias)
        self.q_proj = BlockLinear(embed_dim, embed_dim, bias=bias)
        self.out_proj = BlockLinear(embed_dim, embed_dim, bias=bias)
    ...
    def forward(
        ...
        if (
            ...
            if self.use_xformers:
                ...
                if len(self.cache) > 0 and not self.training:
                    q_weight = self.cache['q_weight']
                    q_bias = self.cache['q_bias']
                    k_weight = self.cache["k_weight"]
                    k_bias = self.cache['k_bias']
                    v_weight = self.cache['v_weight']
                    v_bias = self.cache['v_bias']
                    out_weight = self.cache['out_weight']
                    out_bias = self.cache['out_bias']
                else:
                    ...
                    self.cache['q_weight'] = q_weight
                    self.cache['q_bias'] = q_bias
                    self.cache["k_weight"] = k_weight
                    self.cache['k_bias'] = k_bias
                    self.cache['v_weight'] = v_weight
                    self.cache['v_bias'] = v_bias
                    self.cache['out_weight'] = out_weight
                    self.cache['out_bias'] = out_bias
