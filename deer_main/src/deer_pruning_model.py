...
from .deer_pruning_transformer_layers import TransformerPruningSharedLayer as TransformerSharedLayer
import numpy as np
...
@register_model("deer_pruning_transformer")
class DEERPruningTransformerModel(BaseFairseqModel):
    ...
    def __init__(self, args, encoder, decoder):
        ...
        # different granularity threshold for pruning
        self.attn_head_threshold_ratio = {0.0: float("-inf")}
        self.ffn_threshold_ratio = {0.0: float("-inf")}
        self.inf_init = False
        self.train_inint = False

    def share_encoder_decoder_param(self):
        for enc_layer in self.encoder.layers:
            enc_layer.fc1.parm_mask_scores = enc_layer.fc2.parm_mask_scores
        
    def train_init_threshold(self):
        if not self.train_inint:
            self.init_threshold()
            self.train_inint = True

    def init_threshold(self):
        
        attn_head_scores_collect = []
        ffn_param_scores_collect = []
        
        for layer in self.encoder.layers:
            
            # FFN param score:
            fc1_param_score = layer.fc1.get_param_score()
            fc2_param_score = layer.fc2.get_param_score()

            ffn_param_scores_collect.append(fc1_param_score.view(-1))
            ffn_param_scores_collect.append(fc2_param_score.view(-1))

            # ATTN head score
            # for v2 we remove param "layer.self_attn.head_mask_scores"
            k_head_score = layer.self_attn.k_proj.get_head_score()
            q_head_score = layer.self_attn.q_proj.get_head_score()
            v_head_score = layer.self_attn.v_proj.get_head_score()
            out_head_score = layer.self_attn.out_proj.get_head_score()

            attn_head_scores_collect.append(k_head_score.view(-1))
            attn_head_scores_collect.append(q_head_score.view(-1))
            attn_head_scores_collect.append(v_head_score.view(-1))
            attn_head_scores_collect.append(out_head_score.view(-1))

        attn_head_scores = torch.sigmoid(torch.cat(attn_head_scores_collect, dim=-1)).detach()
        ffn_scores = torch.sigmoid(torch.cat(ffn_param_scores_collect, dim=-1)).detach()

        for ratio in [0.0, 0.25, 0.5, 0.75]:
            attn_head_value = attn_head_scores.kthvalue(int(attn_head_scores.size(0) * ratio) + 1).values
            ffn_value = ffn_scores.kthvalue(int(ffn_scores.size(0) * ratio) + 1).values

            self.attn_head_threshold_ratio[ratio] = attn_head_value
            self.ffn_threshold_ratio[ratio] = ffn_value
    ...
    def reg_threshold(self):
        ...
        for layer in self.encoder.layers:
            attn_head_scores_collect.append(layer.self_attn.k_proj.get_head_score().view(-1))
            attn_head_scores_collect.append(layer.self_attn.q_proj.get_head_score().view(-1))
            attn_head_scores_collect.append(layer.self_attn.v_proj.get_head_score().view(-1))
            attn_head_scores_collect.append(layer.self_attn.out_proj.get_head_score().view(-1))

            ffn_scores_collect.append(layer.fc1.get_param_score().view(-1))
            ffn_scores_collect.append(layer.fc2.get_param_score().view(-1))
        
        # TODO concat all to average
        attn_head_mask_scores = torch.sigmoid(torch.cat(attn_head_scores_collect, dim=-1))
        attn_numel = attn_head_mask_scores.numel()
        attn_head_module_regu = torch.norm(attn_head_mask_scores.float(), p=1)

        ffn_mask_scores = torch.sigmoid(torch.cat(ffn_scores_collect, dim=-1))
        ffn_numel = ffn_mask_scores.numel()
        ffn_module_regu = torch.norm(ffn_mask_scores.float(), p=1)

        module_regu = (attn_head_module_regu + ffn_module_regu) / (attn_numel + ffn_numel)
        return module_regu
    ...
    def show_parameter(self):
        ...
        for layer_index, layer in enumerate(self.encoder.layers):
            ...
            k_mask = layer.self_attn.get_param_number()['k_proj'][0].view(-1)
            q_mask = layer.self_attn.get_param_number()['q_proj'][0].view(-1)
            v_mask = layer.self_attn.get_param_number()['v_proj'][0].view(-1)
            out_mask = layer.self_attn.get_param_number()['out_proj'][0].view(-1)

            fc1_mask = layer.fc1.get_param_number()[0].view(-1)
            fc2_mask = layer.fc2.get_param_number()[0].view(-1)

            layer_attn_param = k_mask.size(0) + q_mask.size(0) + v_mask.size(0) + out_mask.size(0)
            layer_ffn_param = fc1_mask.size(0) + fc2_mask.size(0)

            layer_attn_select_param = k_mask.sum(0) + q_mask.sum(0) + v_mask.sum(0) + out_mask.sum(0)
            layer_ffn_select_param = fc1_mask.sum(0) + fc2_mask.sum(0)
            
            all_param += layer_attn_param + layer_ffn_param
            all_select_param += layer_attn_select_param + layer_ffn_select_param

            # print("attn", layer_index, layer_attn_param, layer_attn_select_param, layer_attn_select_param/layer_attn_param)
            # print("ffn", layer_index, layer_ffn_param, layer_ffn_select_param, layer_ffn_select_param/layer_ffn_param)
        print("all:", all_select_param, all_param, all_select_param/all_param)

    def random_threshold(self):
        
        sampling_ratio = np.random.choice([0.0, 0.25, 0.5, 0.75], size=1, replace=True, p=[1/4] * 4)[0]
        attn_ratio = sampling_ratio
        ffn_ratio = sampling_ratio

        new_attn_head_threshold = self.attn_head_threshold_ratio[attn_ratio]
        new_ffn_threshold = self.ffn_threshold_ratio[ffn_ratio]

        for layer_index, layer in enumerate(self.encoder.layers):

            layer.self_attn.set_head_threshold(new_attn_head_threshold)
            layer.fc1.set_threshold(new_ffn_threshold)
            layer.fc2.set_threshold(new_ffn_threshold)
            
    def inf_by_threshold(self):
        
        if not self.inf_init:
            self.init_threshold()
        ...
        for layer_index, layer in enumerate(self.encoder.layers):
            ...
            layer.self_attn.pruning_parameter()
            layer.fc1.pruning_parameter()
            layer.fc2.pruning_parameter()
...
@register_model_architecture("deer_pruning_transformer", "deer_pruning_transformer")
...
@register_model_architecture("deer_pruning_transformer", "deer_pruning_transformer_large")
